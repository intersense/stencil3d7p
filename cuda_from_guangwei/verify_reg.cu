...
int main(int argc, char* *argv){
    ...
    */
    ...
    for(int t = 0; t < timesteps; t += 1) {
        ...
        jacobi3d_7p_reg<<<grid, block>>>(d_dA, d_dB, nx, ny, nz, fac);
    ...
    printf("Elapsed Time:%lf\n", elapsedTimeC);
    float flops1 = xyz * 7.0 * timesteps;
    float gflops = flops1 / elapsedTimeC / 1e9;
    ...
    printf("-----------------------------------\n");
    printf("resultC[%d]:%f\n", 0+ny*(0+nz*0), resultC[0+ny*(0+nz*0)]);
    printf("resultG[%d]:%f\n", 0+ny*(0+nz*0), resultG[0+ny*(0+nz*0)]);
