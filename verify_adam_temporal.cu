...
int main(int argc, char* *argv){
    ...
    */
    ...
    const int sharedMemSize = 3 * (block.x + 4) * (block.y + 4) * sizeof(float);
    double startTime = rtclock();
    // Run the GPU kernel
    for(int t = 0; t < timesteps; t += 2) {         
        jacobi3d_7p_shmem_adam_overlap<<<grid, block, sharedMemSize>>>(input, output, nx, ny, nz, fac);
    ...
    printf("h_dB[%d]:%f\n", 2+nx*(3+ny*4), h_dB[2+nx*(3+ny*4)]);
    printf("h_dA[%d]:%f\n", 2+nx*(3+ny*4), h_dA[2+nx*(3+ny*4)]);
    printf("h_dB1[%d]:%f\n", 2+nx*(3+ny*4), h_dB1[2+nx*(3+ny*4)]);
    printf("h_dA1[%d]:%f\n", 2+nx*(3+ny*4), h_dA1[2+nx*(3+ny*4)]);
    printf("-----------------------------------\n");
    printf("h_dB[%d]:%f\n", 3+nx*(4+ny*5), h_dB[3+nx*(4+ny*5)]);
    printf("h_dA[%d]:%f\n", 3+nx*(4+ny*5), h_dA[3+nx*(4+ny*5)]);
    printf("h_dB1[%d]:%f\n", 3+nx*(4+ny*5), h_dB1[3+nx*(4+ny*5)]);
    printf("h_dA1[%d]:%f\n", 3+nx*(4+ny*5), h_dA1[3+nx*(4+ny*5)]);
    // Free buffers
    free(h_dA);
    free(h_dB);
    free(h_dA1);
