...
int main(int argc, char* *argv){
    ...
    */
    ...
    dim3 block(tx + 2*(TIME_TILE_SIZE), ty + 2*(TIME_TILE_SIZE), tz + 2*(TIME_TILE_SIZE));

    
    float *tmp;
    float *tmp1;
    float fac = 6.0/(h_dA[0] * h_dA[0]);
    const int sharedMemSize = (block.x) * (block.y) * (block.z) * sizeof(float);
    double startTime = rtclock();
    // Run the GPU kernel
    for(int t = 0; t < timesteps; t += TIME_TILE_SIZE) {         
        jacobi3d_7p_shmem_adam_overlap<<<grid, block, sharedMemSize>>>(input, output, nx, ny, nz, tx, ty, tz,fac);
